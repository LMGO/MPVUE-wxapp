<template>
  <div>
    <!-- 发表内容填写框 -->
    <div class="content">
      <textarea v-model="marketTheme" type="text" placeholder="主题，求购或出售..." rows="1" maxlength="15" style="height:30px; border-bottom: 1px solid rgb(235, 233, 233); ;"></textarea>
      <textarea v-model="marketDes" type="text" placeholder="出售或寻购物品描述..." rows="5" maxlength="200"></textarea>
    </div>
    <!-- 本地图片添加区 -->
    ...
    export default {
      ...
      methods:{
        ...
        publish(){
          ...
          let _this = this;
          // _this.marketTime = _this.$store.state.getTime();
          ...
          _this.$fly.post('http://106.14.83.188:8080/admin/insertmarket',_this.$qs.stringify({
              userId: _this.$store.state.userInfo.userId,
              marketTheme:_this.marketTheme,
              userNickname:_this.$store.state.userInfo.userNickname,
              marketDes: _this.marketDes,
              marketTime:_this.$store.state.getTime(),
